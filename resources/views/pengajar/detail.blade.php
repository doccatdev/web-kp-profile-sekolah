...
@section('content')
    <section class="position-relative" style="margin-top: 76px;">
        <div class="w-100 overflow-hidden" style="height: 350px; position: relative;">
            ...
            <img src="{{ asset('assets/images/school-banner.jpg') }}" class="w-100 h-100 object-fit-cover">
            ...
            <div class="container position-relative h-100 d-flex flex-column justify-content-end pb-4" style="z-index: 2;">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item active text-white-50" aria-current="page">{{ $data->nama }}</li>
                    </ol>
                </nav>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-md-4 text-center mb-4">
                    <img src="{{ $data->foto ? asset('storage/' . $data->foto) : asset('assets/images/default-avatar.jpg') }}"
                        ...
                        style="max-width: 320px; aspect-ratio: 3/4; border: 5px solid #f8f9fa" alt="{{ $data->nama }}">
                </div>

                <div class="col-md-8">
                    <h2 class="fw-bold mb-4">{{ $data->nama }}</h2>

                    <div class="card border-0 bg-light rounded-4 p-4 shadow-sm mb-4">
                        <div class="row g-3">
                            <div class="col-sm-4 fw-bold text-secondary">Jabatan</div>
                            <div class="col-sm-8">: {{ $data->jabatan }}</div>

                            <div class="col-sm-4 fw-bold text-secondary">Mata Pelajaran</div>
                            <div class="col-sm-8">: {{ $data->mata_pelajaran ?? '-' }}</div>
                        </div>
                    </div>

                    <div class="mt-4">
                        <a href="{{ route('profil.data-guru') }}" class="btn btn-secondary btn-sm rounded-pill px-4">
